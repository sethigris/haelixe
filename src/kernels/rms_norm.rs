...
pub fn rms_norm_backward(
    ...
    (0..batch_size).into_par_iter().for_each(|i| {
        unsafe {
            ...
            for j in 0..hidden_dim {
                ...
                let dx_val = (g_val * w_val * inv_rms) - (x_val * inv_rms.powi(3) * dot / hidden_dim as f32);
                *dx_row.add(j) = dx_val;
    ...
    let dw = Tensor::from_slice(DType::F32, w_cpu.shape.clone(), &dw_mutex.into_inner().unwrap());
